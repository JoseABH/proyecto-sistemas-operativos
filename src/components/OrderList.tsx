...
const OrderList: React.FC<OrderListProps> = ({ order, onRemoveItem, onClearOrder, onConfirmOrder }) => {
    return (
        <div className=" bg-gray-100 p-4 rounded-lg shadow-md min-h-60 max-h-60">
            ...
            {order.items.length === 0 ? (
                ...
                <ul className="flex overflow-x-scroll ">
                    {order.items.map(item => (
                        <li key={item.id} className="flex flex-col justify-between items-center border-b p-4 m-1 bg-gray-300 rounded-md min-w-44">
